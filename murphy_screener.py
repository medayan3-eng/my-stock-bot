...
def get_intermarket_regime():
    ...
    # Step 1 (Murphy Playbook) headline call: is the overall environment
    # Risk-On or Risk-Off? Simple proxy: the broad stock market itself is
    # not in a clear downtrend.
    risk_on = stocks != "down"

    return {"trends": trends, "trends_pct": trends_pct, "description": regime,
            "favored_sectors": favored, "risk_on": risk_on}
...
def score_stock(ticker, df, sector, sector_leaderboard, regime, is_etf=False, spy_close=None):
    ...
    # -------------------------------------------------------------------
    # 5-Step Murphy Playbook Checklist (top-down: Intermarket -> Sector ->
    # Trend/Support -> Candlestick timing -> Risk management), matching the
    # exact morning-routine methodology: Murphy's Intermarket Analysis for
    # macro/sector, Nison's candlesticks for entry timing, and Douglas/
    # Schwager-style risk discipline for the final sizing gate.
    # -------------------------------------------------------------------
    trend_confirmed = trend_pts >= 18  # above 52w-ago price AND above both 50/200-day MA
    step1_pass = bool(regime.get("risk_on", True))
    step2_pass = True if is_etf else (sector_pts == 10)
    step3_pass = trend_confirmed and support_signal
    step4_pass = candle_signal
    step5_pass = (not np.isnan(rr_ratio)) and rr_ratio >= 2.0

    checklist = [
        {"step": 1, "label": "Macro/Intermarket", "passed": step1_pass,
         "detail": "Market environment is Risk-On" if step1_pass else "Market environment is Risk-Off — extra caution"},
        {"step": 2, "label": "Sector Strength", "passed": step2_pass,
         "detail": ("N/A for ETFs" if is_etf else
                    (f"Sector ranks in the top third ({sector_rank})" if step2_pass else "Sector is not currently a leader"))},
        {"step": 3, "label": "Trend & Support", "passed": step3_pass,
         "detail": "Confirmed uptrend, currently at a support/pullback zone" if step3_pass
                   else "Missing confirmed uptrend and/or a support test right now"},
        {"step": 4, "label": "Candlestick Timing", "passed": step4_pass,
         "detail": ("Bullish reversal candle detected: " + ", ".join(candle_hits)) if step4_pass
                   else "No bullish reversal candlestick pattern today"},
        {"step": 5, "label": "Risk Management", "passed": step5_pass,
         "detail": (f"R:R ≈ {rr_ratio:.1f} (≥2.0 target met), stop-loss defined" if step5_pass
                    else "Reward:risk below the 1:2 minimum for this setup")},
    ]
    ...
    return {
        ...
        "Checklist": checklist,
        "ChecklistPassCount": checklist_pass_count,
        "Reasons": reasons,
    }
...
def run_scan(tickers, top_n=None, only_strong_sectors=True, min_beta=1.0, only_actionable=True,
             require_volume_spike=False, require_full_checklist=False):
    ...
    print("Step 1 call: Risk-ON" if regime.get("risk_on") else "Step 1 call: Risk-OFF (extra caution)")
    ...
    if require_full_checklist:
        print("(Only stocks passing ALL 5 Murphy Playbook checklist steps are shown — the tightest filter)")

    spy_df = fetch_history(BENCHMARK)
    spy_close = spy_df["Close"] if spy_df is not None else None

    stock_results, etf_results = [], []
    skipped_weak_sector = skipped_beta = skipped_no_signal = skipped_no_vol_spike = skipped_checklist = 0
    ...
    if require_full_checklist and skipped_checklist:
        print(f"Skipped {skipped_checklist} stocks that didn't pass all 5 checklist steps.")
    ...
    def show_and_save(results, label, filename, sort_by_setup=False):
        ...
        df_out = sort_key(df_out) if sort_by_setup else df_out.sort_values(
            ["ChecklistPassCount", "Score"], ascending=[False, False])
        if top_n:
        ...
        export_df["Checklist"] = export_df["Checklist"].apply(
    ...
    if stocks_df is not None:
        ...
        for _, row in stocks_df.head(5).iterrows():
            ...
            print(f"  Murphy Playbook checklist: {row['ChecklistPassCount']}/5 steps passed")
            for c in row["Checklist"]:
                mark = "[PASS]" if c["passed"] else "[FAIL]"
                print(f"    {mark} Step {c['step']} - {c['label']}: {c['detail']}")
            print("  Reasons:")
...
def parse_args():
    ...
    p.add_argument("--full-checklist-only", action="store_true",
                    help="Only include stocks passing ALL 5 steps of the Murphy Playbook checklist "
                         "(macro, sector, trend & support, candlestick, risk management) — the tightest "
                         "filter, for a narrow 3-5 stock watchlist (off by default)")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    run_scan(tickers, top_n=args.top, only_strong_sectors=not args.all_sectors,
              ...
              require_full_checklist=args.full_checklist_only)
